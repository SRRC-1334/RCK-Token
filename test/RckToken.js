...
contract('RckToken', function(accounts) {
  ...
  it('initializes the contract with the correct values', function() {
    return RckToken.deployed().then(function(instance) {
      ...
      assert.equal(name, 'Rck Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'RCKT', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'Rck Token v1.0', 'has the correct standard');
    });
  })
